    ...
    >
      ...
      >
        ...
        <div
          ...
          style={{
            background:
              "radial-gradient(58% 40% at 50% 50%, color-mix(in oklab, var(--navy) 12%, transparent) 0%, transparent 70%), radial-gradient(120% 90% at 50% 50%, transparent 55%, color-mix(in oklab, var(--sepia) 22%, transparent) 100%)",
          }}
        />

        {/* navy floral corner ornaments on the envelope */}
        {position === "top" && (
          <>
            <img
              src={navyFloral}
              alt=""
              aria-hidden="true"
              className="pointer-events-none absolute -top-8 -left-8 w-32 opacity-35 mix-blend-multiply sm:w-40"
            />
            <img
              src={navyFloral}
              alt=""
              aria-hidden="true"
              className="pointer-events-none absolute -top-8 -right-8 w-32 -scale-x-100 opacity-35 mix-blend-multiply sm:w-40"
            />
          </>
        )}
        {position === "bottom" && (
          <>
            <img
              ...
              alt=""
              aria-hidden="true"
              className="pointer-events-none absolute -bottom-8 -left-8 w-32 -scale-y-100 opacity-35 mix-blend-multiply sm:w-40"
            />
            <img
              src={navyFloral}
              alt=""
              aria-hidden="true"
              className="pointer-events-none absolute -bottom-8 -right-8 w-32 -scale-100 opacity-35 mix-blend-multiply sm:w-40"
            />
          </>
        )}
        ...
        <div className="relative flex h-full flex-col items-center justify-center px-10 text-center">
          <p className="caps text-[0.55rem] text-navy/90 font-medium">Holy Matrimony</p>
          <Flourish className="mt-3 w-32 text-gold/70" />
          <p className="script mt-4 text-[3rem] leading-tight text-ink sm:text-[3.75rem]">
            You&apos;re invited
          </p>
          <p className="caps mt-3 text-[0.5rem] text-navy font-semibold">{invite.groom} &amp; {invite.bride}</p>
          ...
          <p className="caps mt-5 text-[0.6rem] text-ink/80">{invite.dayLine}</p>
          <p className="caps mt-3 text-[0.48rem] text-sepia/75">{invite.ceremony.name}</p>
          <p className="caps mt-2 text-[0.44rem] text-navy/60 font-medium">Shillong, Meghalaya</p>
        </div>
      </div>
